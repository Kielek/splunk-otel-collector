...
DEFAULT_SYSTEMD_CONF_PATH = "/usr/lib/systemd/system.conf.d/00-splunk-otel-auto-instrumentation.conf"
DEFAULT_PROFILE_PATH ="/etc/profile.d/00-splunk-otel-auto-instrumentation.sh"
...
@pytest.mark.parametrize("first_package", [PRELOAD_PKG_NAME, ENVVAR_PKG_NAME])
@pytest.mark.parametrize("arch", ["amd64", "arm64"])
def test_package_conflict(distro, first_package, arch):
    ...
    second_package = ENVVAR_PKG_NAME if first_package == PRELOAD_PKG_NAME else PRELOAD_PKG_NAME
